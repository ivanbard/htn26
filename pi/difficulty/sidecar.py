...
if __package__:
    ...
    from difficulty.contract import (
        DIFFICULTIES,
        SCHEMA_VERSION,
    ...
    class DifficultyHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            try:
                ...
                self._json(
                    503, {"status": "unavailable", "error": "model inference failed"}
                )
                return
            self._json(200, response)

        def log_message(self, format, *args):
            print(f"difficulty-sidecar: {self.address_string()} - {format % args}")
...
def parse_args():
    ...
    parser.add_argument(
        "--host", default=os.environ.get("HTN26_DIFFICULTY_BIND_HOST", "127.0.0.1")
    )
    parser.add_argument(
        "--port", type=int, default=int(os.environ.get("HTN26_DIFFICULTY_PORT", "8790"))
    )
    parser.add_argument(
        "--model",
        default=os.environ.get(
            "HTN26_DIFFICULTY_MODEL", str(directory / "difficulty.xml")
        ),
    )
...
def main():
    ...
    print(
        f"HTN26 difficulty sidecar listening on http://{args.host}:{server.server_port}"
    )
    print(
        f"model: {predictor.metadata['name']} {predictor.metadata['version']} ({predictor.artifact_path})"
    )
    try:
