...
local function add_counters(left, right)
	if left >= MAX_COUNTER - right then
		return MAX_COUNTER
	end
	return left + right
end
...
local function valid_player_packet(payload)
	...
	if length < MIN_PACKET_LENGTH or length > MAX_RADIO_PAYLOAD then
		return false
	end
	if string.sub(payload, 1, 4) ~= "OC1|" then
		return false
	end
	if string.find(payload, string.char(0), 1, true) ~= nil then
		return false
	end
	if string.find(payload, "\r", 1, true) ~= nil then
		return false
	end
	if string.find(payload, "\n", 1, true) ~= nil then
		return false
	end
	local kind, sequence, value = string.match(payload, "^OC1|([NMBHEVARU])|(%d%d%d%d)|([^|]+)$")
	if kind == nil then
		sequence, kind, value = string.match(payload, "^OC1|([0-9]+)|([NMBHEVARU])|([^|]+)$")
	end
...
local function enqueue_packet(mac, rssi, payload)
	...
	if queue_tail > QUEUE_CAPACITY then
		queue_tail = 1
	end
...
local function dequeue_packet()
	...
	if queue_head > QUEUE_CAPACITY then
		queue_head = 1
	end
	queue_count = queue_count - 1
...
local function receive_packet(mac, rssi, payload)
	...
	if type(mac) ~= "string" or type(rssi) ~= "number" or not valid_player_packet(payload) then
...
local function update_display(force)
	...
	if
		force
		or shown_invalid ~= invalid_count
		or shown_queue_drops ~= queue_drop_count
		or shown_radio_drops ~= radio_drop_count
	then
		detail_label:set_text(
			"Bad: "
				.. tostring(invalid_count)
				.. "   Queue: "
				.. tostring(queue_drop_count)
				.. "   Radio: "
				.. tostring(radio_drop_count)
		)
		shown_invalid = invalid_count
		shown_queue_drops = queue_drop_count
		shown_radio_drops = radio_drop_count
	end
	if force or shown_last_payload ~= last_payload or shown_last_mac ~= last_mac or shown_last_rssi ~= last_rssi then
		if last_payload == nil then
			...
			source_label:set_text("From " .. last_mac .. "  " .. tostring(last_rssi) .. " dBm")
		end
...
function on_tick()
	...
	if now >= next_status_ms then
		...
		badge.sys.log(
			"HTN26|GW|"
				.. (radio_enabled and "UP" or "DOWN")
				.. "|"
				.. tostring(forwarded_count)
				.. "|"
				.. tostring(total_drop_count())
		)
	end
end
