...
packets use the radio HAL. The root laptop server also opens its configured
serial device read-only. Repeated stock-console `Unrecognized command` messages
therefore indicate that some other host process, terminal echo, bridge, or noisy
connection is delivering bytes to the badge console; they are not generated by
a DROP payload.
