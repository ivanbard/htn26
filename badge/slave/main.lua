...
local ITEMS = {
  BUN = true, RAW_MEAT = true, MEAT = true,
  RAW_LETTUCE = true, LETTUCE = true,
  RAW_CHEESE = true, CHEESE = true, BURNT = true,
}

local ITEM_SHORT = {
  BUN = "B", RAW_MEAT = "R", MEAT = "M", BURNT = "X",
  RAW_LETTUCE = "Q", LETTUCE = "L", RAW_CHEESE = "K", CHEESE = "C",
}

local SHORT_ITEM = {
  B = "BUN", R = "RAW_MEAT", M = "MEAT", X = "BURNT",
...
local function plate_letter(item)
  if item == "BUN" then return "B" end
  if item == "MEAT" then return "M" end
  if item == "LETTUCE" then return "L" end
  if item == "CHEESE" then return "C" end
  return nil
end
...
local function plate_summary(plate)
  ...
  return (plate.B and "B" or "-") .. (plate.M and "M" or "-") ..
    (plate.L and "L" or "-") .. (plate.C and "C" or "-")
end
...
local function plate_from_summary(value)
  ...
  for i = 1, 4 do
    ...
    if c ~= "-" and c ~= letters[i] then return nil end
    if c ~= "-" then plate[c] = true end
  end
  return plate
end
...
local function take_source(state, item)
  if not ITEMS[item] then return false, "BAD_ITEM" end
  if state.chop then return false, "CHOPPING" end
  if state.plate then
    if not is_platable(item) then return false, "NOT_PLATABLE" end
    local letter = plate_letter(item)
    if state.plate[letter] then return false, "DUPLICATE" end
    state.plate[letter] = true
    return true, "PLATE_ADD"
  end
  if state.hand then return false, "HAND_FULL" end
  state.hand = item
  return true, "PICKUP"
end

local function create_plate(state)
  if state.chop or state.plate then return false, "PLATE_ALREADY_HELD" end
  if state.hand then
    if not is_platable(state.hand) then return false, "NOT_PLATABLE" end
    local plate = new_plate()
...
local function add_to_plate(state, item)
  if not state.plate then return false, "NO_PLATE" end
...
local function start_chop(state, now)
  if state.chop then return false, "CHOPPING" end
  if state.hand ~= "RAW_MEAT" and state.hand ~= "RAW_LETTUCE" and
      state.hand ~= "RAW_CHEESE" then
...
local function chop_step(state, now)
  if not state.chop then return 0 end
...
local function finish_chop(state, now)
  ...
  if item == "RAW_MEAT" then state.hand = "MEAT"
  elseif item == "RAW_LETTUCE" then state.hand = "LETTUCE"
  elseif item == "RAW_CHEESE" then state.hand = "CHEESE"
  else return false, "BAD_CHOP" end
  state.chop = nil
  return true, "CHOP_DONE"
end

local function fail_chop(state)
  if not state.chop then return false, "NO_CHOP" end
...
local function stove_action(state, index, now)
  if index ~= 1 and index ~= 2 then return false, "BAD_STOVE" end
  if state.chop then return false, "CHOPPING" end
  ...
  if not state.hand and phase ~= "EMPTY" then
    if phase == "COOKING" then return false, "NOT_DONE" end
    state.hand = phase == "BURNT" and "BURNT" or "MEAT"
    stove.item, stove.started = nil, 0
    return true, phase == "BURNT" and "TAKE_BURNT" or "TAKE"
  end
  if phase == "EMPTY" then return false, "EMPTY_STOVE" end
  if state.hand then return false, "HAND_FULL" end
  return false, "NOT_DONE"
end

local function snapshot(state)
  if state.plate then return "P" .. plate_summary(state.plate) end
  if state.hand then return "H" .. (ITEM_SHORT[state.hand] or "?") end
  if state.chop then return "H" .. (ITEM_SHORT[state.chop.item] or "?") end
  return "E----"
end
...
local function parse_snapshot(value)
  ...
  if kind == "P" then
    ...
    if plate then return { plate = plate, hand = nil } end
  elseif kind == "H" then
    local item = SHORT_ITEM[string.sub(value, 2, 2)]
    if item then return { plate = nil, hand = item } end
  elseif value == "E----" then
...
local function apply_transfer(state, peer)
  if not peer then return false, "BAD_SNAPSHOT" end
  ...
  if local_plate and not peer_plate then
    if peer.hand and is_platable(peer.hand) then
      ...
      if not state.plate[letter] then state.plate[letter] = true end
...
local function event_payload(sequence, player, action)
  if type(sequence) ~= "number" or sequence < 1 or sequence > MAX_SEQUENCE or
      sequence ~= math.floor(sequence) then return nil, "BAD_SEQUENCE" end
  if player ~= 1 and player ~= 2 and player ~= 3 then return nil, "BAD_PLAYER" end
  if not printable_action(action) then return nil, "BAD_ACTION" end
  local payload = string.format("OC1|%06d|E|P%d:%s", sequence, player, action)
  if #payload > RADIO_BYTES then return nil, "TOO_LONG" end
  return payload
end

local function parse_event(payload)
  if type(payload) ~= "string" or #payload > RADIO_BYTES then return nil end
  ...
  if not sequence or sequence < 1 or not printable_action(action) then return nil end
...
local function parse_control(payload)
  ...
  if sequence then return tonumber(sequence), code end
  sequence, code = string.match(payload, "^OC1|(%d+)|G|(START)$")
  if sequence then return tonumber(sequence), "S" end
  sequence = string.match(payload, "^OC1|(%d+)|G|(END)$")
  if sequence then return tonumber(sequence), "E" end
  return nil
end

local function resolve_tag_action(tag, left, right, down, a)
  if not SOURCE_TAGS[tag] then return nil, "UNKNOWN_TAG" end
  if tag == "pantry" then
    if down then return "PLATE", nil end
    if left then return "LETTUCE", nil end
    if right then return "BUN", nil end
    return nil, "UNKNOWN_BUTTON_COMBO"
  end
  if tag == "fridge" then
    if left then return "RAW_MEAT", nil end
    if right then return "RAW_CHEESE", nil end
    return nil, "UNKNOWN_BUTTON_COMBO"
  end
  if tag == "cutting board" then
    if a then return "CHOP", nil end
    return nil, "UNKNOWN_BUTTON_COMBO"
  end
  if tag == "stove" then
    if left then return "STOVE_LEFT", nil end
    if right then return "STOVE_RIGHT", nil end
    return nil, "UNKNOWN_BUTTON_COMBO"
  end
end
...
local function set_status(message, detail, color)
  ...
  if detail and radio_label then radio_label:set_text(detail) end
  status_color = color or 0xffffff
  led_mode, led_until, next_led = (color == 0xff5555 and "error" or "event"),
    badge.sys.ms() + 650, 0
end
...
local function render_state(now)
  ...
  stove_label:set_text("Stoves L " .. left_phase .. " " .. tostring(left_step) .. "/6  R " ..
    right_phase .. " " .. tostring(right_step) .. "/6")
end
...
local function render_leds(now)
  ...
  if setup_mode or not active then
    if math.floor(now / 400) % 2 == 0 then badge.led.set_all(0, 35, 100) end
  elseif led_mode == "error" and now < led_until then
    if math.floor(now / 120) % 2 == 0 then badge.led.set_all(220, 0, 0) end
  elseif state.chop then
    local step = chop_step(state, now)
    for i = 1, step do badge.led.set(i, 220, 150, 0) end
  elseif led_mode == "event" and now < led_until then
    badge.led.set_all(0, 150, 60)
  else
    if player_no == 1 then badge.led.set_all(80, 0, 80)
    elseif player_no == 2 then badge.led.set_all(0, 80, 50)
    else badge.led.set_all(0, 45, 120) end
    local phase = stove_phase(state.stoves[1], now)
    if phase == "WARNING" or stove_phase(state.stoves[2], now) == "WARNING" then
      if math.floor(now / 180) % 2 == 0 then badge.led.set_all(220, 30, 0) end
    end
  end
  badge.led.show()
end
...
local function next_sequence()
  if sequence >= MAX_SEQUENCE then return nil end
  sequence = sequence + 1
...
local function emit(action)
  if not radio_enabled or player_no < 1 or player_no > 3 then return false end
...
local function mark_ready(number, now)
  if number >= 1 and number <= 3 then ready_until[number] = now + READY_MS end
end

local function all_players_ready(values, now)
  for i = 1, 3 do
    if not values[i] or values[i] < now then return false end
  end
  return true
end

local function maybe_commit_submission(now)
  if not active or not submit_record or submission_committed or now > submit_record.deadline then return end
  if not all_players_ready(ready_until, now) then return end
...
local function handle_peer_event(sequence_number, number, action, now)
  if number == player_no or number < 1 or number > 3 then return end
  if sequence_number <= (last_peer_sequence[number] or 0) then return end
...
local function handle_radio_payload(payload, now)
  ...
  if control_sequence and (code == "S" or code == "E") then
    if code == "S" then start_game(control_sequence, code)
    else end_game(control_sequence, code) end
    return
  end
  local sequence_number, number, action = parse_event(payload)
  if sequence_number then handle_peer_event(sequence_number, number, action, now) end
end
...
local function begin_transfer(now)
  if not active or state.chop then return end
...
local function handle_shake(now)
  if b_down then
    handle_drop(now)
  elseif a_down then
    if state.plate then submit_shake(now)
    else begin_transfer(now) end
  else
    ready_shake(now)
  end
end
...
local function handle_pantry(now)
  ...
  if choice == "PLATE" then
    ...
    if not ok then show_invalid(result) return end
    if result == "PLATE_ADD" then emit("PL:" .. plate_summary(state.plate))
    else emit("PU:" .. ITEM_SHORT[item]) end
    set_status(result == "PLATE_ADD" and "ADDED TO PLATE" or "PICKED UP " .. item,
      "Local capture; Pi receives the event stream", 0x8ed8ff)
  end
end
...
local function handle_cutting_board(now)
  ...
  if choice ~= "CHOP" then show_invalid(reason) return end
...
local function handle_stove(now)
  ...
  if result == "PUT" then emit("ST:" .. side .. ":P")
  elseif result == "TAKE_BURNT" then emit("ST:" .. side .. ":X")
  else emit("ST:" .. side .. ":T") end
  set_status(result == "PUT" and "MEAT ON STOVE " .. side or
      (result == "TAKE_BURNT" and "BURNT MEAT - DROP IT" or "COOKED MEAT TAKEN"),
    "Stove " .. side .. " " .. phase .. " " .. tostring(step) .. "/6", 0x8ed8ff)
end
...
local function handle_tag(text, now)
  ...
  if text == "pantry" then handle_pantry(now)
  elseif text == "fridge" then handle_fridge(now)
  elseif text == "cutting board" then handle_cutting_board(now)
  elseif text == "stove" then handle_stove(now)
  else show_invalid("UNKNOWN_TAG") end
end
...
local function poll_nfc(now)
  ...
  if nfc_blocked_uid == card.uid then return end
...
local function service_radio(now)
  ...
  while drained < MAX_RADIO_DRAIN and inbox_count > 0 do
    ...
    for i = 1, inbox_count - 1 do inbox[i] = inbox[i + 1] end
...
local function service_chop(now)
  if not state.chop then return end
  if not a_down then
    local ok = fail_chop(state)
    if ok then emit("CH:F") set_status("CUT FAILED", "A was released before all 6 steps", 0xff5555) end
    return
  end
  local step = chop_step(state, now)
...
local function provision_player()
  ...
  if type(player_no) ~= "number" or player_no < 1 or player_no > 3 then player_no = 1 end
...
function on_enter(root)
  ...
  if type(sequence) ~= "number" or sequence < 0 or sequence > MAX_SEQUENCE then sequence = 0 end
...
function on_button(button, kind)
  ...
  if kind == K.PRESSED then
    if button == B.LEFT then left_down = true
    elseif button == B.RIGHT then right_down = true
    elseif button == B.DOWN then down_down = true
    ...
    elseif button == B.B then b_down = true
  ...
  elseif kind == K.RELEASED then
    if button == B.LEFT then left_down = false
    elseif button == B.RIGHT then right_down = false
    elseif button == B.DOWN then down_down = false
    elseif button == B.A then
      ...
      a_down = false
    elseif button == B.B then b_down = false end
  end
  if setup_mode and kind == K.PRESSED then
    if button == B.LEFT then player_no = (player_no + 1) % 3 + 1
    elseif button == B.RIGHT then player_no = player_no % 3 + 1 end
    set_status("CHOOSE PLAYER " .. tostring(player_no), "LEFT/RIGHT choose; A saves fixed number", 0x8ed8ff)
  end
end
...
function on_exit()
  ...
  if nfc_enabled then badge.nfc.disable() nfc_enabled = false end
  badge.led.clear()
  badge.led.show()
end
...
function on_tick()
  ...
  service_radio(now)
  if nfc_clear_at ~= 0 and now >= nfc_clear_at then badge.nfc.clear() nfc_clear_at = 0 end
  poll_nfc(now)
  service_chop(now)
  if active then
    if badge.sensor.tap() then handle_tap(now) end
    if badge.sensor.shake() then handle_shake(now) end
  end
