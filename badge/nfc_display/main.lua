...
local function is_empty_text_error(error_text)
	...
	if type(error_text) ~= "string" then
		return false
	end
	local normalized = string.lower(error_text)
	return normalized == "no-text"
		or normalized == "no text"
		or normalized == "empty"
		or normalized == "empty tag"
		or normalized == "no ndef text record"
end
...
local function show_empty(uid, error_text)
	...
	record_label:set_text(error_text and "NDEF TEXT:\n(read error)" or "NDEF TEXT:\n(no Text record)")
	detail_label:set_text(error_text and clean_detail(error_text) or "No NDEF Text record found")
...
local function poll_nfc(now)
	if not nfc_enabled or now < next_poll then
		return
	end
	next_poll = now + NFC_POLL_MS

	local card = badge.nfc.card()
	if clear_at ~= 0 and now >= clear_at and (not card or card.uid == seen_uid) then
		badge.nfc.clear()
		rearm_uid = seen_uid
		clear_at = 0
		return
