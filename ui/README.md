...
- `src/transport.js` includes the local HTTP/SSE transport for the production laptop server. It expects `GET /api/state`, `POST /api/command`, and `GET /api/events`.
...
- `src/App.js` contains the React component tree for the staged setup flow, framed room board, order HUD, event-inferred player tokens, fixed-bottom player state cards, and station overlays.
...
- `gameplay`: one framed room board with up to four active orders across the top, fixed-bottom held/action/submission cards, the score at bottom-left, the round clock at bottom-right, and event-inferred player tokens. Operator health chrome is intentionally excluded from the player-facing HUD.
...
- `players[].position`, `location`, `heldItem`, `inventory`, and `actionState` come from the laptop's badge-event projection. Fixed-bottom cards render held/action/submission state; map tokens never claim camera tracking and never animate between snapshots.
...
Tests cover the setup/gameplay/results display modes, aligned station and placement data, event-inferred player positions, held/action/submission cards, invalid normalized coordinates, host scan/approval/start/end/reset transitions, invalid actions, duplicate deliveries, and serving/score success and failure updates. They are host-side behavior checks and do not validate physical badges, bumping, phone-camera capture, or QNX.
