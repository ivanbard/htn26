...
function phaseLabel(phase) {
  if (phase === SETUP_PHASES.SCANNING) return "STATIC FLOOR PLAN REVIEW";
  if (phase === SETUP_PHASES.LAYOUT_PROPOSED) return "STATIC FLOOR PLAN READY";
...
function renderHostPanel(state) {
  ...
  return `<section class="host-panel panel" aria-labelledby="host-title">
    ...
    <div class="setup-controls" aria-label="Host controls">
      ...
      ${commandButton(state, GAME_ACTIONS.SCAN_ROOM, "Review Fixture")}
      ${commandButton(state, GAME_ACTIONS.APPROVE_LAYOUT, "Approve Layout")}
      ${commandButton(state, GAME_ACTIONS.RESCAN, "Reload Fixture")}
...
function renderFloorPlan(state, now) {
  ...
  return `<section class="map-panel panel" aria-labelledby="map-title">
    <div class="section-heading"><div><p class="eyebrow">STATIC ROOM FIXTURE</p><h2 id="map-title">${accepted ? "Accepted burger floor plan" : "Static four-station floor plan"}</h2></div>${statusPill(accepted ? "healthy" : "degraded", accepted ? "authoritative layout" : "awaiting approval")}</div>
    <div class="floor-plan ${accepted ? "is-accepted" : "is-proposed"}" role="img" aria-label="${accepted ? "Accepted burger" : "Static four-station"} top-down station floor plan">
      <div class="map-grid" aria-hidden="true"></div>${walls}${stations}
      ${accepted ? "" : "<div class=\"map-overlay\">APPROVE STATIC FLOORPLAN TO SHOW PLACEMENT</div>"}
    </div>
    <div class="map-legend" aria-label="Floor plan legend"><span>Static stations and walls supplied by the UI fixture</span></div>
  </section>`;
}
...
function renderBurgerPlacement(state) {
  ...
  if (!level || level.status === "not-ready") return "";
  ...
  return `<section class="placement-panel panel" aria-labelledby="placement-title">
    ...
    <p class="muted">Use these fixed locations for the first test build. Place the printed ingredient icons, stationary boards, stoves, and serving badge before starting.</p>
